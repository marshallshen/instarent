...
module Instarent
  class Craiglist
    attr_reader :city, :bathrooms, :bedrooms, :min_price, :max_price
    ...
    def run
      RECIPIENTS.each {|recipient| mailer.notify(apartments, recipient)} unless apartments.empty?
    end
    ...
    def apartments
      ...
      urls = nodes.compact!.map{|l| l.attributes["href"].value}[0..RESULT_SIZE]

      # 2. fetch apt with geo location: [{"apt_url": [123, 321]}]
      candidates = urls.reduce([]) do |candidate_pools, apartment_url|
      ...
      candidates.reduce(Hash.new{|k,v| k[v] = []}) do |apts_per_neighborhood, candidate|
